...
data class DuelAxis(
     ...
     * 형제 축의 표식을 내린다).
     *
     * ⚠️ **정확히는 `(세계관, [targetType])`당 하나다** — 저장소가 지키는 범위가 그것이고
     * (`DuelAxisDao.clearBasisExcept`), 아래 *"이미지 축에만 뜻이 있다"*와 한 벌로 읽어야 한다.
     * 세계관 하나로 뭉쳐 세면 **뜻 없는 캐릭터 축의 표식이 뜻 있는 이미지 축의 표식을 밀어낸다**
     * (2026.08.11 B-118 구현이 실제로 그렇게 썼다가 콜드 검토에서 잡혔다 — 받아온 쪽의 대표 그림
     * 추첨이 조용히 종전처럼 돌아가는 모양이었다). DB 유니크 인덱스로 걸지 않은 것은 *"켜진 것이 없음"*이
