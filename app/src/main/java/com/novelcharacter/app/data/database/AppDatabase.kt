...
import com.novelcharacter.app.data.dao.ImageMetaDao
    ...
    companion object {
        ...
        private val MIGRATION_30_31 = object : Migration(30, 31) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL("ALTER TABLE timeline_events ADD COLUMN eventType TEXT NOT NULL DEFAULT ''")
        ...
        private val MIGRATION_35_36 = object : Migration(35, 36) {
            override fun migrate(db: SupportSQLiteDatabase) {
                Log.i(TAG, "Migrating database from version 35 to 36 — operation_logs (작업 이력)")
                db.execSQL("""
                ...
                """)
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_operation_logs_createdAt` ON `operation_logs`(`createdAt`)")
        ...
        private val MIGRATION_38_39 = object : Migration(38, 39) {
            override fun migrate(db: SupportSQLiteDatabase) {
                Log.i(TAG, "Migrating database from version 38 to 39 — 캐릭터 목록 프리셋에 작품 필터(novelIdsJson) 컬럼 추가")
                db.execSQL("ALTER TABLE `character_list_presets` ADD COLUMN `novelIdsJson` TEXT NOT NULL DEFAULT '[]'")
        ...
        private val MIGRATION_39_40 = object : Migration(39, 40) {
            override fun migrate(db: SupportSQLiteDatabase) {
                Log.i(TAG, "Migrating database from version 39 to 40 — image_meta/image_tags (이미지 라이브러리)")
                // image_meta 먼저(image_tags가 FK 참조)
                db.execSQL("""
                ...
                """)
                db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_image_meta_path` ON `image_meta`(`path`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_image_meta_linkGroupId` ON `image_meta`(`linkGroupId`)")
                db.execSQL("""
                    CREATE TABLE IF NOT EXISTS `image_tags` (
                        `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                        `imageId` INTEGER NOT NULL,
                        `tag` TEXT NOT NULL,
                        FOREIGN KEY(`imageId`) REFERENCES `image_meta`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE
                    )
                """)
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_image_tags_imageId` ON `image_tags`(`imageId`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_image_tags_tag` ON `image_tags`(`tag`)")
                db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_image_tags_imageId_tag` ON `image_tags`(`imageId`, `tag`)")
        ...
        private val MIGRATION_46_47 = object : Migration(46, 47) {
            override fun migrate(db: SupportSQLiteDatabase) {
                Log.i(TAG, "Migrating database from version 46 to 47 — 캐릭터 대표 이미지(B-103)")

                // 순수 추가다. 기존 행은 빈 문자열 = 지정 없음이 되어 종전과 같이 랜덤 표시로
                // 동작한다(소급 지정 없음 — 지금까지 '첫 번째'가 암묵적 대표였을 뿐 사용자가
                // 고른 적이 없으므로, 0번을 대표로 심으면 정하지 않은 것을 정해 주는 것이 된다).
                db.execSQL("ALTER TABLE `characters` ADD COLUMN `representativeImagePath` TEXT NOT NULL DEFAULT ''")
        ...
        private val MIGRATION_50_51 = object : Migration(50, 51) {
            override fun migrate(db: SupportSQLiteDatabase) {
                Log.i(TAG, "Migrating database from version 50 to 51 — 목록 프리셋의 대결 정렬 축(B-117)")

                // 칸 하나를 더하는 **순수 추가**다. 기존 프리셋은 NULL(대결 정렬이 아님)이 되어
                // 종전과 똑같이 동작한다.
                //
                // **NULL을 허용하는 것이 요점이다.** 형제 칸인 `sortFieldKey`도 그렇고, 뜻이
                // *"이 프리셋은 대결로 정렬하지 않는다"*라 빈 문자열로 채우면 **코드가 빈 축을
                // 가리키는 상태**와 구별되지 않는다. 저장 형식이 두 사실을 갈라 두어야
                // 정렬이 "축을 못 찾았다"를 사용자에게 말할 수 있다(개발 의도 2번).
                db.execSQL("ALTER TABLE `character_list_presets` ADD COLUMN `sortDuelAxisCode` TEXT")
        ...
        private val MIGRATION_51_52 = object : Migration(51, 52) {
            override fun migrate(db: SupportSQLiteDatabase) {
                Log.i(TAG, "Migrating database from version 51 to 52 — 대결 축의 프로필 필드(B-122)")

                // 칸 하나를 더하는 **순수 추가**다. 기존 축은 `[]`(프로필 없음)이 되어 종전과
                // 똑같이 동작한다 — 소급해서 필드를 이어 주지 않는 것은 v49→50과 같은 근거다:
                // 어느 필드가 그 축의 *알아보는 재료*였는지 앱이 알 길이 없고, 지어내면
                // 사용자가 정하지 않은 것을 정해 주는 것이 된다.
                //
                // **영향·산출과 한 칸에 합치지 않는 것**도 그때와 같은 근거다. 셋은 쓰임이
                // 갈리고(프로필은 보이되 예측에 안 들어간다), 한 칸에 담고 표식으로 가르면
                // 그 표식을 아는 자리가 코드 전체로 퍼진다.
                db.execSQL("ALTER TABLE `duel_axes` ADD COLUMN `profileFieldKeys` TEXT NOT NULL DEFAULT '[]'")
        ...
        private val MIGRATION_54_55 = object : Migration(54, 55) {
            override fun migrate(db: SupportSQLiteDatabase) {
                Log.i(TAG, "Migrating database from version 54 to 55 — 대결 축의 후보 필터(B-168)")

                // 칸 하나를 더하는 **순수 추가**다. 기존 축은 NULL(필터 없음 = 세계관 전원이
                // 후보)이 되어 종전과 똑같이 동작한다 — 소급해서 필터를 지어 주지 않는 것은
                // v49→50과 같은 근거다.
                //
                // **NULL 허용에 기본값을 두지 않는 것이 요점이다.** 형제 칸 `sortDuelAxisCode`
                // (v50→51)와 같은 모양이고, `'{}'` 기본값을 박으면 휴지통 payload(Gson)가
                // 구버전 스냅샷을 되살릴 때의 null과 저장 형식이 갈려 두 표기가 공존한다 —
                // 해석기(`DuelCandidateFilter.parse`)가 null·빈 값·`{}`를 전부 *필터 없음*으로
                // 읽으므로 저장은 한 가지(null)로 둔다.
                db.execSQL("ALTER TABLE `duel_axes` ADD COLUMN `candidateFiltersJson` TEXT")
         ...
         *
         * 종전에는 `addMigrations(...)` 인자에만 있어서, 마이그레이션을 하나 만들고
         * **거기 등재하는 것을 잊어도 아무것도 알려 주지 않았다** — 증상은 그 버전에서
         * 앱이 죽는 것이고, 그때는 이미 사용자 기기다. 목록을 밖으로 빼면
         * `AppDatabaseMigrationTest`가 실제 Room으로 그 사슬을 끝까지 밟아 본다.
         *
