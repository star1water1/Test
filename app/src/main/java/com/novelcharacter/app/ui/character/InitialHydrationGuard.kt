 ...
 *
 * ## 부르는 쪽의 계약 둘 — **덜 알리는 것보다 더 알리는 것이 위험하다**
 *
 * 이 판정기는 *"만졌다"*를 받으면 그 칸의 적재를 **막는다.** 그래서 **잘못 알리면 유실의
 * 방향이 뒤집힌다** — 덮어쓰기를 막으려다 *DB 값이 아예 안 들어오는* 상태를 만든다.
 * 그 상태로 저장하면 종전 값이 지워진다.
 *
 * 워처가 없는 칸(스피너·이미지)은 특히 조심할 것. **터치만 보면 안 된다** —
 * 드롭다운을 열었다 그냥 닫아도 *골랐다*가 되고, 그러면 적재가 작품·유형을 못 채운
 * 채로 저장이 열린다(콜드 검토 2026.08.19에 실측으로 잡았다). **터치와 그 뒤의
 * `onItemSelected`가 둘 다 있어야** 사용자 선택이다.
 *
