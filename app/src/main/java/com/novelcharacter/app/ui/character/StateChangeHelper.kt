...
class StateChangeHelper(
    ...
    private fun showStateChangeDialog(existingChange: CharacterStateChange?) {
        ...
        //
        // 닿는 길이 실재한다: **필드 정의를 지우면** 그 키를 든 상태변화가 목록 밖으로
        // 떨어진다. 세계관을 옮긴 캐릭터의 옛 세계관 필드도 같다 — 선택지는 *지금 이
        // 캐릭터가 가진 필드*이지 값이 가리키는 정의가 아니기 때문이다(그쪽을 선택지에
        // 넣으면 소속되지도 않은 세계관의 필드를 고를 수 있게 된다).
        //
