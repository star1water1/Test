...
class ImageManagerFragment : Fragment() {
    ...
    private fun showAiTagReview(result: com.novelcharacter.app.ai.ImageBatchTagSuggester.Result) {
        ...
        if (result.suggestions.isEmpty() && notices.isEmpty()) {
            ...
            //
            // 폴더판([OrganizeFolderController])도 같은 자리에서 같은 한 줄을 말한다
            // (2026.08.24 정정 — 종전에는 "폴더 받아오기에 딸린 곁가지라 사용자가 그것만
            // 기다리고 있지 않다"며 폴더판만 침묵해도 옳다고 봤다. 그런데 폴더판 체크박스도
            // **비용을 고지하고 확인받아 사용자가 명시로 켠 유료 동작**이라 이쪽과 무게가
            // 같았고, 실사용에서 그 침묵이 "안 되는 것"과 구별되지 않는다는 보고로 확인됐다).
            //
            // 시트를 여는 대신 한 줄로 말하는 이유는 그대로다 — 고를 것이 없는 창을 세우면
            // 그 자체가 소음이다.
            //
