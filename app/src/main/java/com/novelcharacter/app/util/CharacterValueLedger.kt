 ...
 * 아래 성질들은 어기면 **조용히** 틀리는 부류라 특히 그렇다.
 *
 * ## 지켜야 하는 것 — 세는 법은 `CharacterValueLedgerTest`의 절 머리다
 *
 ...
 * 4. **캐릭터끼리 섞이지 않는다.** 필드 id는 캐릭터 사이에서 공유되므로(같은 필드 정의를
 *    여럿이 쓴다) 키가 필드만이면 한 캐릭터의 값이 다른 캐릭터의 답이 되고, **두 값 모두
 *    그럴듯해 보인다.**
 * 5. **가져오기마다 비운다** — [reset].
 *
 * ## 메모리 — **봉우리는 안 올리되 바닥은 올릴 수 있다**
 *
 * 가져오는 **동안**의 최대량은 *건드린 캐릭터의 값 전부*이고, 그 값들이 실려 온 시트는
 * `StreamingImportWorkbook`이 **이미 통째로** 들고 있다(값 한 건이 한 행/한 칸이므로
 * 시트 쪽이 언제나 크다). 즉 봉우리는 이미 서 있는 상한(*가장 큰 시트 하나*) 안에 들어간다.
 *
 * **그 다음이 함정이다.** 이 장부를 든 서비스는 화면과 수명이 같아
 * (`ExcelTransferController` → `ExcelImporter` → `ExcelImportService`), 가져오기가 끝나도
 * 놓지 않으면 **다음 가져오기까지 그대로 남는다.** 형제 상태들과 달리 담는 것이 id가 아니라
 * **행 전체**(문자열 포함)라 규모에서 자릿수가 다르다. 봉우리를 올리지는 않지만 **바닥을
 * 올려**, 곧이어 도는 내보내기·자동 백업의 봉우리가 그만큼 위에서 시작한다 —
 * 그 둘이 B-72가 지키는 바로 그 경로다. **그래서 호출부는 끝에서도 [reset]을 부른다.**
 */
