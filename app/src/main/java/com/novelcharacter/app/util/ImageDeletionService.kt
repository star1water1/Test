...
object ImageDeletionService {
     ...
     *        **0**이 된다. 낡은 토큰을 그대로 넘겨도 `WHERE linkGroupId = <그 토큰>`에 걸리는
     *        행이 없어 **0행 UPDATE**이고, 항목마다 새로 읽어 `null`을 넘기는 것과 결과가 같다.
     *        (반대 방향으로 낡는 일은 없다 — 삭제 경로는 토큰을 **풀기만** 하고 붙이지 않는다.)
     *        **그래서 이 자리에는 [ImageLinkGroupPlanner] 같은 겹이 필요 없다.** 그쪽 겹이
