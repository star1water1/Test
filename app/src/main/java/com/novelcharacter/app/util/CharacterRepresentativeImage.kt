...
object CharacterRepresentativeImage {
     ...
     *
     * **[characterId]는 '캐릭터'라는 뜻이 아니라 *독립 키*다.** 같은 시드 아래에서 서로 다른
     * 줄이 서로 다른 답을 받게 하는 값이면 무엇이든 된다 — `AiImageAttach`가 목록 위치를
     * 넣고, 작품·세계관 카드가 각자의 id를 넣는다(B-106 ⓑ). 이름을 그대로 두는 것은
     * 호출부와 시험이 이름 붙은 인자로 쓰고 있어서이고, **뜻은 이 문단이 정한다.**
     *
     * **[size]를 그때그때 받는 것이 요점이다** — 종전 작품·세계관 카드는 인덱스를 **저장**해
     * 두고 `idx % size`로 꺼냈고, 그래서 목록이 줄면 **다른 그림**을 가리켰다. 여기서는
     * 고를 때마다 현재 크기로 접으므로 그 자리가 없다.
     */
