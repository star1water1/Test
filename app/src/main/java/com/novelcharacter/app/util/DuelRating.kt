...
object DuelRating {
     ...
     *
     * **공개인 것은 점수를 되돌려 강함으로 읽는 자리가 생겼기 때문이다**(B-104 소비처 ⓑ):
     * 대표 추첨의 가중치는 `10^((점수-앵커)/SCALE)`, 곧 여기서 나온 γ 그 자체다
     * ([RepresentativeWeighting]). 그쪽에 400을 다시 적으면 눈금이 두 벌이 되어,
     * 이 값을 고치는 날 가중치만 조용히 옛 눈금으로 남는다(R-7).
     */
