...
object DuelImageRoster {
     ...
     *
     * ## ⚠️ 나누는 대조는 정규 경로이고, 그 뒤 적합의 대조는 **원본 문자열**이다
     * 여기서는 [ImagePathMatch.canonical]로 주인을 찾지만, 이 산출물을 받는
     * [DuelRecords.resolve]는 참가자를 **글자 그대로** 대조한다. 그래서 판에 적힌 표기가
     * 목록의 표기와 갈려 있으면 **여기서는 그 캐릭터 몫으로 나뉘고 적합에서는 고아가 된다** —
     * [build]의 `played`(고르는 화면)와 [DuelScoreIndex.Entry.played](걸러낼 후보의 최소 판 수)가
     * 그만큼 어긋날 수 있다. **실사용에서는 두 표기가 같다**(판을 적는 자리가 목록의 문자열을
     * 그대로 쓴다) — 갈리는 것은 엑셀·백업으로 손편집된 경로뿐이고, 같은 성질이
     * [com.novelcharacter.app.data.repository.DuelRepository.stateOf]에도 **이미 있다**(순위표가
     * 그 경로다). 한쪽만 정규화하면 순위표와 소비처가 다른 수를 갖게 되어 계약 1이 깨지므로,
     * **여기서 고치지 않고 백로그에 등재했다**(B-174 — 「세션 착수 규칙」 2번: 범위 밖은 등재만).
     *
