...
object DisplayCap {
    ...
    /**
     * 서사 밀도 곡선이 그리는 **칸 수**의 상한 ([NarrativeDensityCurve]).
     *
     * **이 표에서 유일하게 비용이 항목 수가 아니라 *연도 폭*에 붙던 자리다.** 다른 상한들은
     * "캐릭터가 많으면"에 걸리는데 여기는 캐릭터도 사건도 둘뿐인 연표가 폭 12만이면 걸린다 —
     * 판타지 역법에서 정당한 데이터다(사건의 `calendarType` 기본값이 `천개력`이다).
     *
     * 200인 것은 **가로 막대 차트가 라벨을 겹치지 않고 세울 수 있는 상한보다 넉넉하고, 한 번의
     * 레이아웃으로 끝나는 수**라서다. 실사용 표본의 연표 폭은 전부 이 안이라 지금 쓰는
     * 사람의 화면은 한 픽셀도 바뀌지 않는다.
     */
    const val DENSITY_CURVE_BUCKETS = 200
     ...
     * 관여 453명이 요약 모드를 켜고 **91명만** 남겼다(같은 모델의 ×1은 158명이다).
     * 즉 데이터가 늘었는데 그림이 성겨지는 구간이 있었다.
     *
     * **값이 종전 전환 문턱과 같은 200인 것은 화면이 달라지는 지점을 그대로 두기 위해서다** —
     * 종전에도 관여 200명에서 "요약 모드"로 넘어갔으므로, **고지가 뜨는 자리가 정확히 같다.**
     * 달라지는 것은 넘어간 *뒤*의 동작뿐이다(술어 → 상한).
     *
     * > **표본의 관여 인물 수는 실측된 적이 없다.** `scalability` 2장이 세는 것은 캐릭터 214명·
     * > 관계 135건이고, 그중 **몇 명이 관계에 등장하는가**는 그 표에 없다. 위 3-12의 158은
     * > 균등 무작위 모델의 값이며 **참값은 214 이하 어디든 될 수 있다** — 즉 실사용 표본이
     * > 이 상한에 닿을 수도 있다. *"지금 쓰는 사람의 화면이 그대로인가"*는 모델이 아니라
     * > 실기기 확인이 답한다(`remaining_work` 3-120 ㄱ).
     */
