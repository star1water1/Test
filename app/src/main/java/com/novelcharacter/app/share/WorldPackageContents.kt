 ...
 *
 * **v6 유지 (2026.08.25): `faction_relationships.json`의 `code` 칸도 판올림이 아니다.**
 * 세력 간 관계가 안정 식별자를 갖게 되면서(스키마 v58) 그 칸을 DTO에 더했는데, **옛 규칙으로
 * 읽어도 틀리지 않는다**: 칸이 없는 옛 패키지는 가져오기가 코드를 새로 발급하고(= 오늘까지의
 * 동작), 칸이 있는 새 패키지를 옛 앱이 읽으면 Gson이 모르는 칸을 버려 역시 새로 발급한다.
 * 어느 쪽도 다른 엔트리의 뜻을 바꾸지 않는다 — R-64의 기준 그대로다.
 *
