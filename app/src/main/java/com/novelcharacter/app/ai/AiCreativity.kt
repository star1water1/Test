 ...
 * 유료 응답에서만 버리던 비대칭을 없앤 것이며, **프롬프트 지시(아래 [promptBlock])는 그대로다.**
 *
...
enum class AiCreativity(val wire: String, val label: String) {
    ...
    /**
     * 지시문 양식의 `{{창작도지시}}` 자리에 펼쳐지는 단계별 지시.
     * [BALANCED]는 빈 문자열 — 추가 규칙이 없고, 그러면 그 줄이 통째로 사라진다.
     *
     * **번호를 매기지 않는다.** 사용자가 규칙 수를 줄이거나 늘리면 박아 둔 번호가 반드시
     * 틀리기 때문이다(`16.`이 열둘 다음에 오거나 두 번 나온다). 이름표 블록은 그 문제가 없다.
     *
     ...
     */
    fun promptBlock(): String = when (this) {
        BALANCED -> ""
        PRECISE -> """
