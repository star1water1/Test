...
data class ExportOptions(
    ...
    companion object {
         ...
         *
         * 여러 시트를 지배하는 스위치는 **지배 범위를 라벨이 말한다**(대결 라벨이 세운 관례) —
         * '필드 정의' 하나가 시트 4장(등급 체계·기본 필드·필드 정의·필드 데이터)을 켜고 끄는데
         * 라벨이 말하지 않으면 사용자는 나머지 3장이 왜 빠졌는지 알 길이 없다.
         */
        val LABELS = arrayOf(
            "세계관", "작품", "캐릭터", "필드 정의 (등급 체계·기본 필드·필드 데이터 포함)",
