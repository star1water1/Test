...
object ExportSpace {
    ...
    /**
     * **잰 몫이 있을 때만 숫자를 말한다** — 없으면 null이고 호출부는 숫자 없는 문구를 쓴다 (B-189).
     *
     * 종전에는 [requiredMegabytes]를 언제나 불렀고, 그 함수는 **0을 1로 올린다.**
     * 그래서 이미지를 끄고 내보내다 공간이 마르면 앱이 *"약 1MB가 필요합니다"*라고 말했다 —
     * 견적이 이미지 몫만 재기 때문에 잰 것이 아무것도 없는데도 그렇다. 사용자는 1MB를 비우고
     * 다시 눌러 또 실패한다(개발 의도 2번 — 거짓 고지). **0을 1로 올리는 것 자체는 옳다**:
     * *잰 결과가 1MB 미만*과 *아무것도 재지 못함*은 다른 사실이고, 갈라야 할 것은 그 둘이다.
     *
     * **어림으로 메우지 않는 것을 고른 이유.** 다른 갈래는 워크북 몫을 행 수로 어림하는
     * 것이었는데, 그 수는 시트 구성·서식·이미지 없는 문자열 길이에 따라 자릿수까지 흔들린다.
     * **틀린 숫자는 숫자가 없는 것보다 나쁘다** — 사용자가 그 수를 믿고 딱 그만큼만 비우기
     * 때문이다(R-14 — 모르는 것을 아는 척하지 않는다). B-72가 시트를 임시 파일로 흘려보내는
     * 축을 새로 붙인 뒤로 어림의 오차는 더 커졌다.
     *
     * @param measuredBytes 이번 내보내기에서 **실제로 잰** 바이트. 재지 못했으면 null.
     */
    fun requiredMegabytesOrNull(measuredBytes: Long?): Int? =
        measuredBytes?.takeIf { it > 0L }?.let { requiredMegabytes(it) }
}
