...
/**
 * 엑셀의 **'텍스트로 저장된 숫자' 표식을 끌 열** — 순수 판정(내보내기가 부른다).
 *
 * ## 왜 끄는가
 *
 * 코드는 UUID의 앞 16자라(`generateEntityCode`) 16글자가 **전부 숫자일 수 있다** —
 * `(10/16)^16` ≈ 4만 3천분의 1이고, 항목이 천 단위면 파일마다 몇 건씩 나온다. 실측:
 * 사용자 파일의 캐릭터 코드 하나가 `2057265523594451`이었다.
 *
 * 그 칸에 엑셀이 녹색 삼각형과 **[숫자로 변환]**을 띄운다. 눌리면 16자리가 배정밀도
 * 부동소수(유효 15자리)로 접혀 **코드가 다른 글자로 바뀐다** — 열 단위로 누르면 그 시트의
 * 정체가 통째로 어긋나고, 가져오기는 *"코드를 찾지 못해 이름으로 매칭"*으로 떨어지거나
 * 남의 행을 가리킨다. 회색이라 *고치지 마세요*라 안내해 둔 칸을 **엑셀이 먼저 고치라고
 * 권하는** 셈이다.
 *
 * ## 무엇을 고르는가 — 둘이다
 *
 * ⓐ **앱이 채우는 회색 글자 열**(`readOnly`이고 [ColumnSpec.millis]가 아닌 것).
 *   millis 열을 빼는 것은 그쪽이 **진짜 숫자**로 나가 애초에 이 표식이 붙지 않아서다.
 *   함께 걸리는 '설명'·'입력 가능한 값' 같은 회색 글자 열에는 켜 두어도 잃을 신호가 없다.
 *
 * ⓑ **이름이 `코드`로 끝나는 열** — 회색이 아니어도. 참조 코드 열 일부는 **일부러 편집
 *   가능**하다(안내: *"참조 코드 열은 이름이 겹칠 때 직접 채워 대상을 확정할 수 있습니다"*).
 *   `이미지캐릭터코드`·`이미지작품코드`가 그것이고, 사용자가 **붙여 넣은** 코드도 같은
 *   위험을 진다. ⓐ만 보면 그 열이 빠진다 — 실제로 `NumberWarningFreeColumnsTest`의 전수
 *   검사가 그 누락을 잡았다.
 *
 * **표식 자체는 쓸모가 있어서 나머지는 켜 둔다** — 사용자가 나이 칸에 숫자를 텍스트로 넣으면
 * 그 경고가 맞다. 이름 규칙에 사용자 필드 열이 걸릴 수는 있으나(`필드:…코드`) 그때 잃는 것은
 * *권유 표식 하나*이고, 반대 방향의 실수는 **되돌릴 수 없다** — 그래서 이쪽으로 기운다.
 */
fun SheetSpec.numberWarningFreeColumns(): List<Int> =
    columns.withIndex()
        .filter { (_, col) -> (col.readOnly && !col.millis) || col.header.endsWith(CODE_COLUMN_SUFFIX) }
        .map { it.index }

/** 코드 열 이름의 꼬리 — 안내 시트가 쓰는 그 말('코드 컬럼'·'참조 코드')과 같은 어휘다. */
private const val CODE_COLUMN_SUFFIX = "코드"
...
fun imageMetaSpec() = SheetSpec(
    ...
    columns = listOf(
        ...
        // **`char:` 토큰이 누구인지 파일 안에서 읽을 수 있게 한다.** 자동 링크 토큰은
        // `char:<캐릭터 내부 id>`인데(`AutoLinkPlanner.AUTO_TOKEN_PREFIX`), 이 워크북의 다른
        // 참조는 전부 16자리 코드라 **그 숫자를 캐릭터로 되짚을 길이 파일 어디에도 없었다**
        // (실측 2026.08.24 사용자 파일: 1,443행 중 1,437행이 `char:273` 꼴이다).
        //
        // 토큰 자체는 그대로 둔다 — 자동 링크 엔진의 불변식이고, 표기를 바꾸면 그 엔진과
        // DAO 질의까지 함께 옮겨야 한다. 대신 **읽기 전용 이름 열**을 곁에 세운다.
        // 형제 시트가 이미 쓰는 관용구다('이름 은행'의 사용 캐릭터 · '작품 필드값'의 작품제목).
        ColumnSpec("링크 캐릭터", readOnly = true, width = 6000),
