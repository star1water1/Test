...
class EventFieldHeadersTest {
    ...
    fun uniqueName_noUniverseSuffix() {
        ...
        assertEquals(listOf("필드:규모"), EventFieldHeaders.headersFor(fields, universes).map { it.second })
    }
    ...
    fun duplicateName_getsUniverseSuffix() {
        ...
        assertEquals(
            ...
            EventFieldHeaders.headersFor(fields, universes).map { it.second }
        )
    }
    ...
    fun parenthesizedFieldName_roundTripsExactly() {
        ...
        val headers = EventFieldHeaders.headersFor(fields, universes)
        assertEquals("필드:규모(명)", headers[0].second)

        val expected = EventFieldHeaders.expectedHeaders(fields, universes)
    ...
    fun universeNameWithParens_roundTripsExactly() {
        ...
        val fields = listOf(field(1, "규모", 1), field(2, "규모", 2))
        val expected = EventFieldHeaders.expectedHeaders(fields, u)
    ...
    fun fallback_exactFieldNameWins_overQualifierGuess() {
        ...
        val parsed = EventFieldHeaders.parseFallback("필드:규모(명)", names, setOf("아스트라"))!!
    ...
    fun fallback_qualifierOnlyWhenUniverseExists() {
        val hit = EventFieldHeaders.parseFallback("필드:규모(아스트라)", emptySet(), setOf("아스트라"))!!
        assertEquals("규모", hit.fieldName)
        assertEquals("아스트라", hit.universeName)

        // 실존 세계관이 아니면 한정자로 보지 않는다 — 이름의 일부로 취급
        val miss = EventFieldHeaders.parseFallback("필드:규모(명)", emptySet(), setOf("아스트라"))!!
    ...
    fun nonFieldHeader_isNotAColumn() {
        assertNull(EventFieldHeaders.parseFallback("연도", emptySet(), emptySet()))
        assertNull(EventFieldHeaders.parseFallback("세계관코드", emptySet(), emptySet()))
    }
}
