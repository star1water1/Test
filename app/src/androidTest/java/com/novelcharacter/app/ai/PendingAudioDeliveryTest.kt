...
class PendingAudioDeliveryTest {
    ...
    @Test fun successRestoresAudioAndEditedTextUntilExplicitAppend()=onMain {
    ...
    @Test fun failuresKeepBytesAndExplicitRetryUsesCurrentProvider()=onMain {
    ...
    @Test fun cancellationAndViewModelClearPreserveAudioAndAllowExplicitRetry()=onMain {
    ...
    @Test fun processDeathInFlightIsVisibleWithoutAutomaticRequest()=onMain {
    ...
    @Test fun transcriptWriteFailureKeepsAudioAndRefusesConsumption()=onMain {
    ...
    @Test fun failedRetranscriptionKeepsPreviousRawAndEdit()=onMain {
        val item=ready();val vm=model(item)
        vm.transcribeFile={_,_,_,_,_->SpeechResult.Success("첫 원문","fake")};vm.transcribe()
        vm.editDraft("첫 원문 직접 수정")
        vm.transcribeFile={_,_,_,_,_->SpeechResult.Failure(SpeechError.SERVER)};vm.transcribe()
