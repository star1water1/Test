...
- **캐릭터 필드 값 추천** (`ai/CharacterFieldAiSuggester` + `ui/character/AiFieldSuggestSheet`) —
  ...
  **중복 없이 선별**한다: 앞 2/3은 빈도 상위(주류 기조), 나머지 1/3은 잔여 목록을 균등 간격으로
  ...
  (`FieldValueRules.validateRestricted`)와 **같은 집합**으로 목록 밖 값을 드롭한다 — 단
  라이브러리가 비어 있으면 제한하지 않는다(허용 목록을 준 적이 없으므로). 허용 목록을 다
  싣지 못한 restricted 필드는 truncationNotes로 결손을 고지한다(R-14).
