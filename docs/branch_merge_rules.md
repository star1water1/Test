...
- ~~주의: 저장소 기본 브랜치가 master가 아닌 동안에는 GitHub이 PR 생성 시 잘못된 base 브랜치를 기본값으로 제안한다.~~ → **전제 해소(5-1 완료, 2026-07-03 — 2026.08.01 상시 규칙에서 정정):** 기본 브랜치가 master가 된 뒤로 이 경고의 조건은 성립하지 않는다. 다만 **PR을 만들 때 base가 `master`인지 확인하는 규칙 자체는 조건 없이 유지한다**(CLAUDE.md도 "저장소 기본 브랜치 설정과 무관하게" 무조건 규칙으로 든다).
...
### 5-2. 미병합 브랜치 7개 처리 `[상태: 완료 (2026-07-03) — 브랜치 삭제는 5-3에서 일괄 수행]`
...
### 5-3. 이미 병합된 브랜치 삭제 `[상태: 완료 (2026-07-03)]`

> **완료:** 소유자가 GitHub 웹에서 `claude/*` 브랜치 전체를 삭제했고, 원격에 `master` 단 하나만 남았음을 확인함(`git ls-remote --heads` 결과 1개). 이로써 일회성 정리 작업(5-1~5-3)이 모두 끝났다 — 이후로는 본 문서의 상시 규칙(1~6)만 따르면 된다.
>
> 아래는 당시 삭제 검증 기록(복구 참고용)이다. 삭제 당시 재검증 결과 42개 전부 안전 판정이었고, Claude Code 원격 환경의 git 프록시가 삭제 push를 차단(403)하여 실행은 소유자가 수행했다.
> *(계수 각주 2026.08.01 — 아래 명령의 브랜치는 **43개**로 판정 수 42와 1 어긋난다. 차이는 정리 기록 자체를 담았던 `claude/branch-cleanup-record`로, 이미 병합 완료라 안전 판정 목록의 대상이 아니었다.)*
...
| 버전 | 날짜 | 변경 내용 |
|------|------|-----------|
| v1.6 | 2026.08.01 | **적대 검토 2-4 반영** — 규칙 3의 기본 브랜치 경고를 전제 해소로 정정(5-1 완료로 조건이 소멸했는데 상시 규칙에 현재형으로 남아 있었다 — base 확인 규칙 자체는 무조건 유지), 5-3 삭제 명령의 계수 각주(43 대 판정 42 — 차이는 정리 기록 브랜치). 5-3의 명령·SHA 기록은 복구 참고용이라 유지한다 |
| v1.5 | 2026.07.31 | **문서 감사 반영** — 5-2 상태 표기 정정: "브랜치 삭제만 남음" → "브랜치 삭제는 5-3에서 일괄 수행"(5-3과의 모순 해소) |
...
| v1.4 | 2026.07.03 | 5-3 완료 — 소유자가 전 브랜치 삭제, 원격에 master만 남음 확인. 일회성 정리 작업(5-1~5-3) 전체 종료 |
