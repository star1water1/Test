...
값의 `description`도 `· 뜻:`으로 함께 실린다 (B-46) — **병합 판단의 직접 근거**라
*"뜻이 다른 두 값을 합치지 마라"*가 규칙으로 붙는다. **자르지 않는다**(이 경로엔 절단 고지
채널이 없다) 대신 **6,000자 예산이 설명 몫을 함께 센다** — 길면 청크가 쪼개져 요청이 늘고
그 수를 실행 전 고지가 그대로 말한다(고지가 `chunkEntries`를 단일 소스로 쓴다).
...
- **값의 뜻**: 값에 쓴 `description`은 예시로 **실린 값에만** `값 뜻` 절로 따라간다 (B-46).
  예산은 예시 목록과 **분리**돼 있어 설명이 길어도 허용 목록이 짧아지지 않는다.
