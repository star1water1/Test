...
- `restricted`: 라이브러리 값만 허용 — 저장 시 검증하되 차단 대신
  ...
  > ⚠️ **경로 간 강도 불일치 — 판정 대기(B-79):** 위 정의대로면 restricted는 "안내 후
  > 수용"인데, **AI 추천 경로는 같은 목록 밖 값을 드롭**하고 그것을 "계약"이라 부른다
  > (`ai_integration`·`ai_control_and_ui_density`). 손으로는 넣을 수 있는 값을 유료 응답에서는
  > 버리는 비대칭 — 어느 강도가 맞는지 하나로 정할 때까지 이 정의(안내 후 수용)가 저장
  > 경로의 현행이다.
