...
## 스키마 (`field_value_entries` — **도입 시점 DB v41.** 현행 버전은 `AppDatabase.kt`의 `version =`이 단일 소스다)
...
이동·세계관 이관·사건 필드값 저장·**작품 필드값 저장**(`harvestForNovel` —
...
> **모집단은 현재 값 세 표뿐이다 — 상태변화 이력은 수확하지 않는다**
> (2026.08.10 · B-60 · 사용자 확정 20번 ㄱ1. 종전에는 이 목록에 **상태변화 기록**이 있었고
> 아래 백필도 *"전체 수확(상태변화 이력 포함)"*이었다).
> **왜 바꿨는가:** 집계(`recountUsageForFieldsOrThrow`)는 처음부터 캐릭터·사건·작품의
> **현재 값**만 셌는데 수확은 이력까지 담아, **수확과 집계가 서로 다른 모집단을 봤다.**
> 증상이 조용하다 — 이력에만 있는 값은 엔트리로 들어오고도 `usageCount`가 **영원히 0**이라
> '미사용 자동수집 정리'가 그것을 지우자고 권한다. 어느 쪽에 맞출지는 *집계의 의미*를
> 정하는 일이라 판정을 받았고, 답은 **`usageCount` = "지금 쓰이는 횟수"**였다.
> 그래서 **수확 쪽을 좁혔다**(집계를 넓히면 카탈로그가 *지금은 아무도 안 쓰는 값*을
> 쓰이는 것으로 센다).
> **대가:** 이력에만 있던 값은 카탈로그에 자동으로 오르지 않으므로 자동완성 후보에서도 빠진다.
> 필요하면 사용자가 직접 등재한다(자율성 우선). **이력 자체는 아무것도 잃지 않는다** —
> 전파는 `character_state_changes`를 그대로 대상에 두므로(아래 「전파」) 표기를 바꾸면
> 이력도 함께 따라간다.
> **걷은 자리는 넷이다:** `harvestForCharacter` · `harvestUniversesOrThrow` ·
> `harvestStateChange`(함수째 삭제 — 존재 이유가 이력 수확 하나였다) · 휴지통의 상태변화 복원.
> **한 자리만 고치면 나머지로 같은 비대칭이 그대로 남는다.**
...
config에서 두 맵 제거) ② 전체 수확(**현재 값만** — 위 상자) ③ 전 필드 재계산. 실패 시 다음 실행 재시도.
...
- `restricted`: 라이브러리 값만 허용 — 저장 시 검증하되 차단 대신
  ...
  > ✅ **경로 간 강도가 하나로 맞춰졌다 (B-79 — 확정 2026.08.02 · 처리 완료 2026.08.03,
  > 세션 로그 1-bh).** **AI 쪽을 완화했다** — 목록 밖 값을 드롭하지 않고 저장 경로와 같은
  > 처분("목록 밖 후보 + 추가 제안")으로 올렸다. 채택한 값은 폼에 들어가고 **저장할 때
  > 기존 restricted 가드가 그대로 그 질문을 한다**(두 번째 '라이브러리에 추가' 자리를 만들면
  > 같은 결정에 경로가 둘이 된다). **완화는 목록 하나에만 걸린다** — 옵션·형식(`INVALID`)은
  > 그대로 거부한다. 그래서 **이 정의(안내 후 수용)가 이제 두 경로의 공통 현행**이다.
  >
  > *(2026.08.04 정정: 이 상자가 **"판정 대기"에 머물러 있었다.** 확정도 구현도 끝난 뒤였고,
  > 더 나쁜 것은 **"AI는 드롭한다"가 현행 동작이 아닌데 현행처럼 적혀 있던 것**이다 —
  > 이 문서는 restricted 정의의 단일 소스라 그 서술이 그대로 근거로 쓰인다.)*
...
## AI 필드 추천의 표기 기조 소스 (`ai/CharacterFieldAiSuggester`)

캐릭터 필드 값 AI 추천이 **이 카탈로그를 기존 표기 기조의 단일 소스로 소비**한다.
필드별 중복 없는 정규 값 + 사용 빈도를 이미 들고 있어, 캐릭터 전체를 훑지 않고
`entriesForFields` 1쿼리 배치 조회로 "이 작품이 이 필드를 어떻게 써 왔는가"를 얻는다.

- **예시 선별**: 숨김 제외(숨김의 계약이 '입력 제안에서 제외'이고 AI 추천도 입력 제안이다),
  빈도 상위 + 균등 샘플로 소수만 프롬프트에 실어 토큰을 통제한다.
- **접기·검증**: 별칭 → canonical 접기와 `restricted` 허용 검증은 숨김 엔트리를 **포함**한
  집합으로 한다 — 저장 시 가드(`FieldValueRules.validateRestricted`)와 같아야 하기 때문.
- **inputMode 존중**: `free`는 용례·접기 모두 하지 않는다.

상세는 docs/ai_integration.md.
...
- **usageCount의 의미 = "지금 쓰이는 횟수"** (2026.08.10 사용자 확정 20번 ㄱ1 · B-60).
  "쓰인 적 있는 횟수"가 아니다. **수확과 집계가 같은 모집단(현재 값 세 표)을 본다**는 것이
  이 결정의 실질이고, 위 「자동 수확」의 상자가 걷은 자리를 든다. 이 정의가 흔들리면
  '미사용 자동수집 정리'의 대상 판정이 함께 흔들린다 — 그 기능이 `usageCount == 0`을 보고
  *"아무도 안 쓰는 값"*이라 말하기 때문이다.
- **자동 고아 정리 잡 없음 — 단 '고아'는 두 종류고 처분이 다르다**(2026.08.01 구분 명시 —
  종전 서술은 둘을 한 단어로 뭉쳐 "구조적으로 안 생긴다"로 읽혔다): ① **FK 고아**(부모 필드
  삭제)는 CASCADE라 생기지 않는다 ② **실사용 고아**(어느 캐릭터도 더는 안 쓰는 AUTO 엔트리)는
  생기고 누적된다 — 그 정리는 자동이 아니라 라이브러리 탭의 "미사용 자동수집 정리"(수동,
  건수 확인 후 실행)가 담당한다. 큐레이션 흔적(라벨·별칭·설명·카테고리·숨김)이 있는 엔트리는
  어떤 정리로도 삭제되지 않는다.
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v1.1 | 2026.08.01 | **적대 검토 2-4 반영** — 스키마 절 제목의 "DB v41"을 도입 시점으로 한정(현행 버전은 세는 곳 위임), restricted 정의에 경로 간 강도 불일치 경고(B-79 판정 대기), 고아 서술을 FK/실사용 두 종으로 분리(종전엔 "안 생긴다"로 뭉쳐 읽혔다), **이력 표 신설**(이 문서만 이력이 없어 서술 시점 판정이 불가능했다 — 이전 개정 이력은 소급 불가) |
