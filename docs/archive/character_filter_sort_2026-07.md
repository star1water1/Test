> ## 📦 보관 — **구현 완료** (2026-07-28 코드 재확인: `util/FieldFilterHelper.kt` · `util/SortComparators.kt` 실재)
>
> 그 뒤 이 축에서 일어난 일 둘: ① 성능 — 키 입력마다 전체 재계산하던 것을 에폭 메모이제이션으로
> (`archive/search_filter_sort_perf_2026-07.md`). ② 짝 맞춤 — 필터는 있는데 정렬이 없는 화면들을
> 전수 조사해 3곳을 채웠다(`docs/filter_sort_parity_2026-07.md`, 남은 격차는 백로그 B-47·B-48).
>
> **현행 사양은 그 둘이다.** 이 문서는 최초 설계의 근거로만 읽을 것.
