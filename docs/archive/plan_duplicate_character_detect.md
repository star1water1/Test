...
> `ui/character/CharacterSaveCoordinator.kt` · `ui/character/DuplicateCharacterDialog.kt` 실재.
>
> **계획서로 읽지 말 것.** 실사용 표본에서 동명이인은 **0/214**였고
> (`docs/usage_reality_check_2026-07.md` §1-C), 동명 판정은 그 뒤 이미지 폴더 왕복에서
> 다른 의미를 얻었다 — 앱이 만든 폴더에서는 동명이 **오묶음**이 되므로 내보내기가 제외한다
> (`docs/image_folder_roundtrip_design_2026-07.md` C-14).
>
> *(파일명 변경 이유: 루트에 `PLAN.md`와 `plan.md`가 대소문자만 다르게 공존해,
> 대소문자를 구분하지 않는 파일시스템에서 체크아웃이 충돌했다.)*
