...
> ## 📦 보관 문서 — 구현이 끝나 `docs/archive/`로 옮겼다. 아래 상태 헤더 참조.

> **상태: 구현 완료** (2026-07-28 코드 재확인) — `util/StorageAnalyzer`가 실재하고 고아 이미지
> 범주(`orphanImages`)를 포함하며, 이 문서가 최대 원인으로 지목한 자동 백업 이미지 포함 기본값은
> **`DEFAULT_INCLUDE_IMAGES = false`로 뒤집혔다**(당시 `true`).
> 이 문서는 **왜 그렇게 됐는지**의 근거로 읽을 것이며, 새 성능 판단의 진입점은
> `docs/scalability_performance_2026-07.md`다.
