> ## 📦 보관 문서 — **2026-03-21 시점 리뷰** (원래 위치: 저장소 루트 `CODE_REVIEW.md`)
>
> **이 문서의 전제가 낡았다.** 아래 머리말은 "122 소스 파일, Room DB v25"라고 적혀 있으나
> 2026-07-28 실측은 **328 파일 / 87,108줄 / DB v44**(엔티티 26 · 마이그레이션 44)다.
>
> **특히 위험한 한 줄:** 4.2 표의 *"테스트: 테스트 파일 미발견 → Unit/Integration 테스트 추가 필수"*.
> 현재는 **테스트 68파일 · 순수 JVM 882건**이 `tools/run_jvm_tests.sh`로 실행된다
> (수식 평가기·마이그레이션 하네스 포함). 이 줄을 근거로 "테스트가 없다"고 판단하면 틀린다.
>
> **잔여 항목 중 2026-07-28에 코드로 재확인해 살아 있는 것**은 백로그로 옮겼다
> (`docs/remaining_work_2026-07.md` 4장 **B-56~B-59**). 그 넷 외의 항목은 **재확인하지 않았다** —
> 착수 전 반드시 현행 코드로 실재를 확인할 것(이 저장소의 규칙: 문서의 지적은 근거가 아니다).
>
> 현행 구조의 지도는 `docs/architecture_2026-07.md`, 성능·확장 한계는
> `docs/scalability_performance_2026-07.md`가 단일 소스다.
