> ## 📦 보관 — **F-1~F-7 구현 완료** (원본 점검 문서가 기록: `app_inspection_round2_2026-07.md` — "문-1~문-7 전체 수정 완료, 2026-07-04, DB v34→v35")
>
> 여기서 나온 것 중 지금도 코드의 뼈대인 것: **안정 식별자 `code`**(사건·상태변화·관계변화 —
> `data/model/EntityCode.kt`). 그 뒤 이 축은 규약 **R-1**(코드가 대상을 정하고 id는 확인 수단)로
> 굳었고, 휴지통 복원·엑셀 왕복이 전부 그 위에 서 있다.
>
> **원본 점검(문제점/개선점/확장점/시나리오 각 7건)은 `docs/app_inspection_round2_2026-07.md`에
> 현행으로 남아 있다.** 이 문서는 그 대응 설계의 근거다.
