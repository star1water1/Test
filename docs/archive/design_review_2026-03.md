> ## 📦 보관 문서 — **2026-03 시점 리뷰** (원래 위치: 저장소 루트 `DESIGN_REVIEW.md`)
>
> **일부는 이미 해결됐고 일부는 살아 있다.** 2026-07-28에 표본 확인한 결과:
>
> | 이 문서의 지적 | 현행 |
> |---|---|
> | 3.6 비교 모드 UX — 모드 진입 피드백 미흡, 선택 수 미표시 | **해결됨** — N-1이 비교 전용 바 신설(취소·선택 수·실행 상주, `CharacterListFragment`) |
> | 3.7 위젯 기능 제한 — `RemoteViewsService` 목록 위젯 없음 | **유효** — 미도입 |
> | 3.8 이미지 관리 — Bitmap 직접 처리 | **부분 해결** — 복붙돼 있던 다운샘플·경로 가드를 `util/CharacterImageLoader`로 모았다. 라이브러리(Coil/Glide) 도입 권고 자체는 유효 |
>
> **나머지 항목은 재확인하지 않았다** — 착수 전 반드시 현행 코드로 실재를 확인할 것.
> 그 뒤 UI/UX 판단은 `docs/usability_review_2026-07.md` ·
> `docs/ai_control_and_ui_density_2026-07.md`(상하단바 밀도) ·
> `docs/text_style_guide_2026-07.md`(문구)가 이어받았다.
