> ## 📦 보관 — **Phase A 구현 완료** (2026-07-28 코드 재확인: `ui/assistant/` 8파일 실재)
>
> 이 문서가 정한 "완전 오프라인 원칙 · 코치+검사기+자동화를 별도 탭으로"는 그대로 살아 있다.
> 그 뒤 AI 축은 `docs/ai_integration.md`와 `docs/ai_control_and_ui_density_2026-07.md`(P-A)가
> 이어받았고, 규약 **R-23**(학습한 사실은 대상이 바뀌면 함께 버린다) · **R-24**(성립하지 않는
> 조합의 설정은 보이지 않는다)가 거기서 나왔다.
>
> Phase B(자동화 제안)는 아직 로드맵이다 — `remaining_work_2026-07.md` 4장 우선순위 5.
