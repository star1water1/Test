> ## 📦 보관 — **구현 완료** (2026-07-28 코드 재확인: `util/ResultNotify.kt` · `util/ResultReporting.kt` 실재)
>
> 이 설계가 세운 것 — "모든 데이터 처리는 결과를 알린다"(변수 제어: 검증 → 알림 → 교정) — 는
> 그 뒤 규약으로 굳었다: **R-17**(빈 결과는 사유를 말한다) · **R-14**(잘라낸 것은 개수로 알린다) ·
> **R-26**(작업형 진행도). 규약 색인은 `docs/architecture_2026-07.md` 6장.
>
> **여기 적힌 절차를 지금 사양으로 읽지 말 것.** 남긴 이유는 "왜 이 앱이 결과를 이렇게까지
> 집요하게 알리는가"의 근거다.
