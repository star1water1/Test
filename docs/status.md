...
- 사용자 지정 신규 작업인 [AI 자연어 일괄편집](ai_natural_language_batch_edit_plan.md)은 M1의 순수 입력·Plan·검토 상태와 M2의 범위 한정 인물/필드 문맥 조회·전용 prompt·`AiService` 분석 경계를 구현했다. 동명이인 인용, 응답 절단·형식 오류는 검토 가능 상태와 분리해 보존한다. 화면·DB 적용·실기기 검증은 아직 연결하지 않았으며, 이후 M3~M7을 순서대로 진행한다.
