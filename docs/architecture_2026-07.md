...
| **신규 요청 5건 로드맵·설계 (2026.08.06)** | **`feature_roadmap_2026-08`** — 전역 기본 필드(B-119) · AI 이미지 첨부(B-120) · 일괄 태깅(B-121) · 대결 프로필+화면 재설계(B-122, B-114 흡수 — 목업 `mockups/duel_play_redesign_2026-08.html`) · 대결 등급 산정(B-113의 답). **설계만 — 구현 대기이며 착수 게이트는 그 문서 10장(Q1~Q6)의 답**이다. 기존·예정 기능 개선 검토(5장)와 구현 순서 초안(9장)도 그 문서가 든다 |
