...
| `DuelScoreIndex` | **대결 점수가 대결 밖에서 무엇으로 보이는가** (B-117) — 순위표의 줄을 밖이 쓰는 표로 옮기는 것 · **무기록 참가자를 값 없음으로 두는 판정** · 값 없음의 정렬 자리 · 분포 구간 | 설계 4장은 *"파생값을 저장하지 않는다 — 저장하면 두 자리가 갈라진다"*고 못 박았는데, **소비처가 셋이 되니 저장하지 않아도 갈라질 수 있었다**(각자 적합하면 옵션 하나에 점수가 달라진다). 저장 금지의 나머지 절반이 **출구를 하나로 묶는 것**이고 이 파일이 그 출구다. 무기록 판정을 여기 둔 것도 같은 이유 — 앵커 1500을 값으로 섞으면 **아무것도 안 한 캐릭터가 목록 한가운데에 박힌다**(코드가 하지 *않는* 일이라 시험으로만 지켜진다) |
| `DuelAiContext` | **대결이 AI 프롬프트에서 무엇으로 보이는가** (소비처 ⓔ) — 실을 축 고르기 · **점수가 있는 참가자끼리 등수 다시 매기기** · 줄의 모양(`강함: 12명 중 3위 · 1523 ±37 (12판)`) · 상한과 뺀 수 고지 | 조립기가 둘이라(짧은 값 추천 · 서술형 작성) **각자 자르면 같은 캐릭터가 경로에 따라 다른 축을 받는다.** 등수를 다시 매기는 것이 이 파일의 핵심 판정이다 — 순위표 줄의 등수는 **한 판도 안 친 참가자까지 세는데** 그들의 앵커 1500이 표 한가운데 앉아, 그대로 옮기면 분모를 넘는 *"3명 중 4위"*가 나온다(설계 10-3. 자기 재검토가 잡았다). 프롬프트 문자열은 **어떤 로컬 검증도 읽지 않으므로** 시험이 유일한 방어선이다 |
