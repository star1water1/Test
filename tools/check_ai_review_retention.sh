...
# 이 조합은 실패가 아니라 정상 경로다 — 프롬프트가 *"근거를 찾을 수 없으면 빈 배열로 둔다"*고
# 시키고 빈 배열은 어디에도 세지 않으므로, 모델이 전부 그렇게 답하면 제안도 고지도 없이 끝난다.
# 그런데 **비용을 고지하고 확인받아 실행한 유료 동작**이라, 그 자리에서 화면이 침묵하면
# 사용자가 보는 것은 아무 변화도 없는 화면뿐이다(고장과 구분되지 않는다 — R-17).
#
# **폴더판도 배치판과 같이 본다(2026.08.24 정정).** 종전에는 "폴더 받아오기에 딸린 곁가지라
# 사용자가 그것만 기다리고 있지 않다"며 이 항목에서 폴더판을 뺐다. 그런데 폴더판 체크박스도
# 요청 건수를 미리 고지하고 사용자가 명시로 체크해 돈을 쓰는 동작이다(같은 R-24·비용 고지
# 계약) — "곁가지라 안 기다린다"는 전제가 실사용에서 틀렸다(사용자 보고: 새 사진을 받아왔는데
# 검토 창이 뜨지 않고 아무 알림도 없었다). 시트를 열지 않는 것은 여전히 옳다(고를 것이 없는
# 창은 소음이다) — 다만 한 줄은 말해야 한다.
...
# 배치판(Fragment)·폴더판(컨트롤러) 둘 다 같은 계약을 진다(2026.08.24 정정 — 위 주석 참조).
for f in "$FRAGMENT" "$FOLDER_SHEET_CALLER"; do
  [ -f "$f" ] || continue
  empty_path_notifies "$f"; rc=$?
  if [ "$rc" -eq 2 ]; then
    echo "  ✗ 빈 결과 경로를 찾지 못했습니다: $f (suggestions.isEmpty() && notices.isEmpty())"
    echo "      → 조건의 모양이 바뀌었으면 이 검사를 함께 고칠 것. 못 찾은 채 통과하면"
    echo "        *위반 없음*과 구별되지 않는다."
    fail=1
  elif [ "$rc" -eq 1 ]; then
    echo "  ✗ 제안도 고지도 없는 실행이 말없이 빠집니다: $f"
