...
# **정규식은 `-v`가 아니라 환경변수로 넘긴다.** `-v pat=...`는 값을 *문자열 리터럴*로 읽어
# 이스케이프를 한 겹 벗기므로, `\(`가 awk의 정규식 컴파일러에는 맨 `(`로 닿아 **짝 없는 괄호**가
# 된다. 그 처분이 구현마다 갈린다 — 로컬 **mawk**는 경고 없이 넘어갔고 CI의 **gawk**는
# `fatal: invalid regexp`로 죽었다(2026.08.12 실측. 이 검사를 세운 그 PR의 첫 CI가 잡았다).
# `ENVIRON`은 그 벗기기를 거치지 않아 두 구현이 같은 정규식을 본다.
# **자기 시험이 이 차이를 큰소리로 냈다는 것도 적어 둔다** — 조용히 통과했다면 이 검사는
# gawk 환경에서 *아무것도 안 보면서 초록*이었을 것이다(이 파일이 막으려는 그 모양 그대로).
readers_in() {
