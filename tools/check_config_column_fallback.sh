...
echo "── '설정(JSON)' 위치 폴백 금지 검사 (R-36 · B-142 · B-162) ──"

# 한 줄이 옳은가: `cols["설정(JSON)"]` 뒤가 `?: -1` 인가.
# (`?:` 없이 쓰는 형태는 타입이 `Int?`라 아래 소비처가 컴파일되지 않으므로 여기서 보지 않는다.)
bad_lines() {
  grep -rn -F "$NEEDLE" --include=*.kt "$1" 2>/dev/null |
    grep -v -F '?: -1'
}
...
EOF
good_hits=$(bad_lines "$SELFDIR/nested/Good.kt" | grep -c . || true)
bad_hits=$(bad_lines "$SELFDIR/nested/Bad.kt" | grep -c . || true)
rm -rf "$SELFDIR"
...
found=$(bad_lines "$SRC" || true)
total=$(grep -rn -F "$NEEDLE" --include=*.kt "$SRC" 2>/dev/null | grep -c . || true)

if [ "$total" -eq 0 ]; then
  echo "  ✗ '설정(JSON)' 열을 푸는 자리를 하나도 찾지 못했습니다" >&2
  echo "      → 헤더 문구가 바뀌었으면 이 검사의 NEEDLE을 함께 고칠 것." >&2
  echo "        찾지 못한 채로 통과하면 **위반이 없는 것과 구별되지 않는다**." >&2
  exit 1
fi

if [ -n "$found" ]; then
  echo "  ✗ '설정(JSON)' 열에 위치 폴백이 남아 있습니다:"
  printf '%s\n' "$found" | sed 's/^/      /'
  echo ""
  echo "      → 없는 열은 -1이고, 그때는 기존 설정을 지킨다(R-36)."
  echo "        위치로 폴백하면 그 열을 지운 파일에서 **이웃 열이 config로 읽혀**"
  echo "        options·formula·등급표를 덮는다(B-162가 그 사고다)."
  echo ""
  echo "'설정(JSON)' 위치 폴백 금지 검사 실패" >&2
  exit 1
fi

echo "  ✓ '설정(JSON)'을 푸는 $total 자리 전부 -1 폴백 (R-36)"
echo ""
echo "'설정(JSON)' 위치 폴백 금지 검사 통과"
exit 0
