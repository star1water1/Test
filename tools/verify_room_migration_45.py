...
def extract_migration_sql(src):
    ...
    start = src.index("MIGRATION_44_45")
...
def main():
    ...
    check(re.search(r"addMigrations\([^)]*MIGRATION_44_45", src, re.S) is not None,
          "MIGRATION_44_45가 addMigrations에 등록됐다 (빠뜨리면 실행 시 IllegalStateException)")
