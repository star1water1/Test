...
#
# **셋으로 늘었다 (2026.08.16, B-205).**
#   [C] **「문서 이력」에 판의 행이 빠졌는가** — 4장이 아니라 그 문서의 이력 표를 본다.
#       `CLAUDE.md` v2.1이 *"2026.08.07 이후는 「문서 이력」이 현행 기록이다"*로 못박고
#       **소급 채움을 금지**했으므로, 그 표는 *"지난 세션이 무엇을 왜 했는가"*의 **유일한**
#       소스다 — 빠지면 다른 벌이 없다. 실제로 **16·17판이 두 판 연속 빠졌고**(2026.08.12)
#       2026.08.16에야 발견됐다. **실패의 모양이 침묵인 부류다**: 안 적어도 아무것도 깨지지
#       않고, [A]·[B]는 4장만 보므로 원리적으로 못 본다.
#       판정은 **번호가 끊겼는가** 하나다 — 「로드맵 N판 구현」의 N이 최소~최대에서 연속이어야
#       한다. *한 판도 안 빠졌다*는 그 문서 안에서 기계로 확인되는 유일한 형태이고,
#       사람이 세지 않아도 된다는 것이 이 검사의 값이다.
#
...
# [C] 「문서 이력」의 로드맵 판 번호가 끊겼는가. 빠진 번호를 한 줄씩 낸다.
gaps() {
  python3 - "$1" <<'PYEOF'
import re, sys
nums = sorted({int(m) for m in re.findall(r'로드맵 (\d+)판 구현', open(sys.argv[1], encoding='utf-8').read())})
# 번호가 하나뿐이거나 없으면 *끊김*이라는 말 자체가 성립하지 않는다 — 통과시킨다.
missing = [n for n in range(min(nums), max(nums) + 1) if n not in nums] if len(nums) > 1 else []
for n in missing:
    print(n)
...
# [C]도 같은 이유로 자기 시험을 먼저 한다 — 정규식이 안 맞으면 "빠진 번호 없음"과 구별되지 않는다.
GAPTEST=$(mktemp)
printf '%s\n' \
  '| v1 | d | **로드맵 1판 구현 — 있다** |' \
  '| v2 | d | **로드맵 2판 구현 — 있다** |' \
  '| v4 | d | **로드맵 4판 구현 — 3판이 빠졌다** |' > "$GAPTEST"
if [ "$(gaps "$GAPTEST")" != "3" ]; then
  echo "  ✗ 탐지기 자기 시험 실패 — 빠진 판 번호를 못 잡습니다" >&2
  rm -f "$GAPTEST"; exit 1
fi
rm -f "$GAPTEST"

GAPS=$(gaps "$DOC") || {
  echo "  ✗ 「문서 이력」에 행이 없는 판:" >&2
  printf '%s\n' "$GAPS" | sed 's/^/      로드맵 /;s/$/판/' >&2
  echo "    → 그 판의 행을 「문서 이력」에 적으십시오. 이 표가 *지난 세션이 무엇을 왜 했는가*의" >&2
  echo "      유일한 현행 소스이고(CLAUDE.md v2.1), 소급 채움이 금지돼 다른 벌이 없습니다." >&2
  exit 1
}
echo "  ✓ 「문서 이력」의 로드맵 판 번호에 빠진 것 없음"
