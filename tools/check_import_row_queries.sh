...
#    **`adopt`·`adoptAuto` 자신이 목록에 든 뒤에도 그대로다** — 그 둘의 선언과 본문이 거기 살고,
#    빼지 않으면 **정의가 제 이름에 걸려** 검사가 영영 빨간불이다(단수판은 지워지지 않는다:
#    한 장을 다루는 것이 일인 자리가 실재하고, 그 자리는 아래 `단발 허용` 표식으로 지난다).
#  · **일괄판이 아예 없는 단수 호출은 이 검사의 부류가 아니다.** ~~`adopt`를 경로마다 부르는
#    루프가 그것이다.~~ **→ 그 예가 없어졌다(2026.08.16 B-244).** `ImageAdoption.adoptAll`이
#    서면서 `adopt`·`adoptAuto`도 *일괄판이 있는 단수형*이 되어 아래 목록에 들어왔다.
#    **이 줄을 지우지 않고 남기는 것은 규칙과 예를 갈라 두려는 것이다** — 규칙은 여전히
#    유효하고(일괄판 없는 단수형은 이 검사가 침묵한다), 낡은 것은 예뿐이다.
...
BANNED = {
    ...
    # `adoptAuto`가 `adopt`보다 **먼저** 와야 한다. 파이썬 정규식의 교대는 왼쪽부터라
    # 순서가 반대면 `adoptAuto(`에서 `adopt`를 먼저 시도한다 — 뒤따르는 `\s*\(`가 없어
    # 되돌아 잡히기는 하지만, 되돌리기에 기대는 순서를 남겨 둘 이유가 없다.
    'adoptAuto':             'ImageAdoption.adoptAllAuto(db, paths, now) — 경로 목록을 한 번에 (B-244)',
    'adopt':                 'ImageAdoption.adoptAll(db, paths, now) — 경로 목록을 한 번에 (B-244)',
}
