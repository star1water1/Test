...
# 문구의 `joinToString(", ")`이 마흔 곳 넘게 있고 그것들은 입력칸이 아니다. 그런데 이 저장소는
...
marked() {  # $1 = 파일, $2 = 줄번호
  sed -n "$2p;$(( $2 - 1 ))p" "$1" 2>/dev/null | grep -qF "$MARK"
}
...
while IFS= read -r hit; do
  ...
  if ! printf '%s' "$body" | grep -qE '\.split\(","\)|\.split\('"'"','"'"'\)|setText\(.*joinToString'; then
